...
exports.createGeneration = function(rpcData, blockReward, charityReward, feeReward, recipients, poolAddress){
    var poolAddrHash = bitcoin.address.fromBase58Check(poolAddress).hash;
    var charityAddrhash = bitcoin.address.fromBase58Check('iHgcntCBzgkVfuWfKVq7gHzPzmp26uyScS').hash;
    ...
    tx.addOutput(
                 scriptCompile(charityAddrhash),
                 Math.floor(charityReward)
                 );
    tx.addOutput(
